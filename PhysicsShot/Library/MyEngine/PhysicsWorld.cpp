...
void PhysicsWorld::DebugPhysicsUpdate()
{
	...
	for (int j = m_dynamicsWorld->getNumCollisionObjects() - 1; j >= 0; j--)
	{
		...
		//---aabb判定ではオブジェクト自体のWorld行列を元に作成しているため,描画の位置を変えるために行列を変更しておく
		//---WorldTransformだけを変えてもそのあと補完する作業が入るらしく描画がおかしくなるので,補完後の行列も強制的に変更する
		obj->setWorldTransform(trans);
		obj->setInterpolationWorldTransform(trans);
